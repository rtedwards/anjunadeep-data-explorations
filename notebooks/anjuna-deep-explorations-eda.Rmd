---
title: "EDA Anjunadeep Explorations"
...
tracks.df <- read.csv(file="data/anjunadeep-explorations-track-audio-features.csv", header=TRUE)
```
...
track_features.df <- tracks.df %>%
  select(popularity, acousticness, danceability, energy, instrumentalness, liveness, speechiness, valence) %>%
  glimpse()
```
...
#  mapping = ggplot2::aes(color = smoker),
  ...
  lower = list(continuous = wrap("points", alpha = 0.25), combo = wrap("dot", alpha = 0.25)),
